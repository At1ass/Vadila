...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->button2->Text = L"Ïåðåäàòü ñëåäóþùåìó";
...
#pragma endregion
    ...
    private: System::Void íîâàÿÈãðàToolStripMenuItem_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		if (valid[0]) {
			...
			label12->Text = "Ñòàâêà:  " + ready_players[index].pay;
		...
		if (valid[1]) { 
			...
			label13->Text = "Ñòàâêà:  " + ready_players[index].pay;
		...
		if (valid[2]) {
			...
			label14->Text = "Ñòàâêà:  " + ready_players[index].pay;
		...
		if (valid[3]) {
			...
			label15->Text = "Ñòàâêà:  " + ready_players[index].pay;
		...
		if (valid[4]) {
			...
			label16->Text = "Ñòàâêà:  " + ready_players[index].pay;
		...
		if (valid[5]) {
			...
			label17->Text = "Ñòàâêà:  " + ready_players[index].pay;
	...
	private: void remove_cell(System::Windows::Forms::DataGridView^ table, System::Windows::Forms::DataGridViewCellEventArgs^ e, unsigned player) {
		...
		if (++winning[player - 1] == 15) {
			...
			label2->Visible = true;
			show_all();
			label2->Text = gcnew String("Èãðîê " + nicks[player - 1] + " ïîáåäèë!!! Âûèãðûø:  " + prize());
		}
