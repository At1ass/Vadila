...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->checkBox1->Text = L"Àâòîìàòè÷åñêàÿ èãðà";
...
#pragma endregion
	...
	private: void remove_cell(System::Windows::Forms::DataGridView^ table, System::Windows::Forms::DataGridViewCellEventArgs^ e, unsigned player) {
		...
		if (++winning[player - 1] == 15) {
			...
			label2->Visible = true;
			label2->Text = gcnew String("Èãðîê " + player.ToString() + " ïîáåäèë!!!");
		}
		...
		if (count == COLS) {
			label9->Visible = true;
			label9->Text = gcnew String("Èãðîê " + player.ToString() + " : Êâàðòèðà");
		}
	}
